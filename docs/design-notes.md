...
HA クラスタの一次/二次ノード名を `pair: { primary, secondary }` 構造で保持。
`MddoL3FirewallPair` は `MddoOspfTimer` と同じパターン (単一ハッシュをサブ属性化) で実装。
...
MddoL3NodeAttribute
  └── firewall: MddoL3Firewall           (ext key: 'firewall', default: {})
        ├── pair: MddoL3FirewallPair      (primary, secondary)
        ├── zones: Array<MddoL3FirewallZone>    (name, interfaces)
        └── policies: Array<MddoL3FirewallPolicy>  (from_zone, to_zone, rules)
